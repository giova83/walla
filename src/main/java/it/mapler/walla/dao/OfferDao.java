	...
	// ritorna un elenco di Offerte
	   ...
	   public void offerAdd(Offer offer) throws WallaDBException {
		...
		String  sqlAdd = "INSERT INTO "+ TABLE_OFFER + 
							" (idofferta, descrizione, categoria, idristorante, tipologia, titolo, datapubblicazione) " +
							"VALUES (?, ?, ?, ?, ?, ?, ?)" ;
		      
		Object[] params = {offer.getIdofferta(), offer.getDescrizione(), offer.getCategoria()
						, offer.getIdristorante(),offer.getTipologia(),offer.getTitolo(),offer.getDatapubblicazione()};   	  
	             
		LOGGER.info(offer +"added to the table " +TABLE_OFFER);
		 
		int[] types = { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.TIMESTAMP };	
